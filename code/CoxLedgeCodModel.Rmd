...
# treating day as a quadratic doesn't substantially improve fit, but it does make more mathmatical sense
...
# This thre multiple errors/warnings related to infinite or missing values, and na function evaluation
...
#nd<-expand.grid(Y=as.character(unique(gsub$Y)),Site=as.character(unique(gsub$Site)),J=275:400,MOON=pi,H=0,WK=NA,DAY=NA,DEPTH=50)
nd<-expand.grid(Y=as.character(unique(gsub$Y)),Site=as.character(unique(gsub$Site)),J=275:365,MOON=pi,H=0,WK=NA,DAY=NA,DEPTH=50)
nd<-make_circ(nd)
nd = cbind(nd,as.data.frame(predict(bbest,newdata=nd,type='link',se.fit = TRUE)))
nd$lwr_linkP = nd$fit + qnorm(0.025)*nd$se.fit # convert se to CI on link scale
nd$upr_linkP = nd$fit + qnorm(0.975)*nd$se.fit 
...
emm_jMB$edate<-as.Date('2010-12-31')+emm_jMB$J
...
#ANNUAL x SITE FX
  ...
  pG<-sub$edate[sub$pG==max(sub$pG)]
    pk_rG<-sub$edate[sub$rG==max(sub$rG)]
    df<-dat  pk_a.frame(Site=sitz[s],peak_pG=pk_pG,peak_rg=pk_rG)
