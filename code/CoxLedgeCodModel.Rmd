...
global_R_cDay = glmmTMB(n_grunts ~ SpawnSeason + sday + cday + sLunar + cLunar + sLunar2 + cLunar2 + shour + chour,
                   data = cox, family = nbinom2(),
                   ziformula = ~.)
...
model.sel(global_R_nb2, global_R_cDay)
...
emm_j_rg<-as.data.frame(emmeans(global_R_cDay,~sday+cday,at=list(J=J,sday=sday,cday=cday)),type='response')
...
ggplot(data = emm_j_rg, aes(x = edate, y = response))+ theme_bw()+
  ...
  theme(axis.title = element_text(size = 14),
        axis.text = element_text(size = 12))+
  ggtitle("Global Model (Day as circular - much higher AIC)")
