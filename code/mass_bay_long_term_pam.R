...
if(TRUE){
  ...
  pam$Time1[pam$Year==2016]<-pam$Clock[pam$Year==2016] #BECAUSE 2016 DATA WERE DIFFERENT
  ...
  pam$PERIOD<-get_period(pam$Datetime) #MJD CUSTOM FUCTION
  pam$MOON<-lunar.phase(as.Date(pam$Date),name=FALSE) #from "lunar" package
  pam$MOON4<-lunar.phase(as.Date(pam$Date),name=4)
  pam$MOON8<-lunar.phase(as.Date(pam$Date),name=8)
  #pam<-subset(pam,Year!=2016)
  
  #SAMPLING EFFORT 
  pam_eff<-read.csv("mass_bay_pam_effort.csv") #DATES MARUS WERE DEPLOYED
  pam_eff$Date<-as.Date(pam_eff$Date)
  hdf<-data.frame(H=0:23)
  mdates<-merge(pam_eff,hdf) #CREATES A ROW FOR EACH HOUR
...
if(TRUE){
  ...
  btab<-btab[order(-btab$CAND,btab$AIC),][c("mod","form","loglik","AIC","dAIC","w","df")]
  btab
  ...
  ztab<-ztab[order(-ztab$CAND,ztab$AIC),][c("mod","form","ziform","loglik","AIC","dAIC","w","df")]
  ztab
  ...
  for(s in 1:length(sitez)){
    ...
    for(y in 1:length(yrz)){
      ...
      if(nrow(pred)>1){
        ...
        plot(obs_GRUNTS~J,data=ag_obs[order(ag_obs$J),],type='h',lwd=2,col="gray",xlim=xl,yaxt='n',xaxt='n',ylim=yl)
  ...
  for(s in 1:length(sitez)){
    ...
    for(y in 1:length(yrz)){
      ...
      if(nrow(pred)>1){
        ...
        ag_obs<-aggregate(obs_GRUNTS~J+MONTH,data=pred,FUN=mean)
        ag_pred<-aggregate(pred_GRUNTS~J+MONTH,data=pred,FUN=mean)
        plot(log(obs_GRUNTS+1)~J,data=ag_obs[order(ag_obs$J),],type='h',lwd=2,col="gray",xlim=xl,yaxt='n',xaxt='n',ylim=yl)
